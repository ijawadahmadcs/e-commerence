...
const Product = ({product}) => {
  return (
    <div className='bg-white p-4 rounded-lg shadow-lg transition 0.3 ease'>
      ...
      <Image width={300} height={150} src={product.image} alt={product.title} className={"w-full h-48 object-cover rounded-md transition 0.3s ease"}/>
  </Link>
  <div className='mt-4'>
...
<div className="mt-3 space-y-1 text-sm text-gray-700">
  <p><span className="font-medium text-gray-900">Color:</span> {product.color}</p>
  ...
  <p><span className="font-medium text-gray-900">Discount:</span> {product.discount}%</p>
</div>
...
<p className="mt-3 text-lg font-bold text-pink-600">
  ${product.price}
     ...
     <button className='bg-pink-00 text-white px-4 py-2 rounder-md hover:bg-pink-700 transition duration-300'>Buy Now</button>
  </div>
    </div>
  )
}
